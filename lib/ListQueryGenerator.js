...
module.exports = class ListQueryGenerator {
  ...
  constructor({
    ...
    this._filters = _(filters)
      ...
      .filter((filter) => {
        const property = _.find(type.properties, { name: filter.name });
        return filter.name === 'id' || (!_.isUndefined(property) && !_.isArray(property.type));
      })
      .value();
    this._orderBy = _.filter(orderBy, (orderBy) => {
      const property = _.find(type.properties, { name: orderBy });
      return orderBy === 'id' || (!_.isUndefined(property) && !_.isArray(property.type));
    });
  ...
  _innerQuery() {
    const orderByColumns = _(this._orderBy)
      .map((orderBy) => `"${orderBy}"`)
      .join(', ');

    const orderByText = this._orderBy.length > 0 ? `ORDER BY ${orderByColumns}` : '';

    return `SELECT * FROM "${this._type.name}" ${orderByText} LIMIT $1 OFFSET $2`;
  }
  ...
  _arrayDataQuery() {
    ...
    const select = _(this._arrayFields)
      .map((arrayField) => `"t${arrayField}".value as "${arrayField}", "t${arrayField}".id as "${arrayField}_int_id"`)
      .join(', ');
  ...
  _expandsQuery() {
    ...
    const select = _(this._expands)
      .flatMap((expand) => {
        ...
        const fields = _(expandType.properties)
          .filter((property) => !_.isArray(property.type))
          .map(
        ...
        return [`"j${expand.name}"."id" as "${expand.name}_id"`, ...fields];
      })
      .join(', ');
