...
module.exports = class ListQueryGenerator {
  ...
  generateListQuery() {
    ...
    const select = ['inner.*', arrayQuery.select].join(', ');

    const fullQuery = `SELECT ${select} FROM (${innerQuery}) as inner ${arrayQuery.join};`;
    return {
      text: fullQuery,
  ...
  _arrayDataQuery() {
    const join = _(this._arrayFields)
      .map((arrayField) => {
        return `LEFT OUTER JOIN "${this._type.name}_${arrayField}" as ${arrayField} ON inner.id = ${arrayField}.${this._type.name}`;
      })
      .join(' ');

    const select = _(this._arrayFields)
      .map((arrayField) => `${arrayField}.value as ${arrayField}`)
      .join(', ');
