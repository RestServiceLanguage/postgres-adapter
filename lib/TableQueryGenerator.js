...
module.exports = class TableQueryGenerator {
  ...
  _generateArrayTableQuery(property) {
    ...
    const refValue = customType ? `REFERENCES "${type.name}"(id) ON DELETE CASCADE ON UPDATE CASCADE` : ''; 

    const query = `CREATE TABLE IF NOT EXISTS "${tableName}" (id SERIAL PRIMARY KEY, "${this._type.name}" INTEGER REFERENCES "${this._type.name}"(id) ON DELETE CASCADE ON UPDATE CASCADE, value ${valueType} ${refValue});`;
